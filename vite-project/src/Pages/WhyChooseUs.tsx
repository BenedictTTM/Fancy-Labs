...
export default function WhyChooseUs() {
  ...
  return (
    <div className="bg-black px-4 py-12 sm:py-16 md:py-20 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div
          ref={headerRef}
          className={`text-center md:text-left mb-10 md:mb-16 transition-all duration-1000 transform ${
            isHeaderVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
          }`}
        >
          <p className="inline-block text-yellow-400 font-medium px-4 py-1 rounded-full bg-yellow-400/10 mb-3 md:mb-4 text-xs sm:text-sm">
            Features
          </p>

          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-gray-100 mb-4 md:mb-6 px-2">
            Why Choose <span className="text-yellow-400">Us</span>?
          </h2>

          <p className="text-gray-400 text-base sm:text-lg mb-4 md:mb-6 max-w-3xl mx-auto md:mx-0 px-2">
...
function AnimatedFeatureItem({ icon, title, description, delay = 0 }: AnimatedFeatureItemProps) {
  ...
  return (
    <div
      ref={featureRef}
      className={`flex flex-col sm:flex-row items-start group transition-all duration-700 transform ${
        isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'
      }`}
    >
      <div className="relative w-full">
        <div className="absolute left-0 w-1 h-full bg-gradient-to-b from-yellow-400 to-yellow-600 rounded-full group-hover:h-3/4 transition-all duration-300 hidden sm:block"></div>
        <div className="sm:ml-6 flex flex-col sm:flex-row items-start">
          <div className="rounded-full bg-gray-800 p-3 sm:p-4 mb-4 sm:mb-0 sm:mr-6 border border-yellow-500/20 shadow-lg shadow-yellow-500/10 group-hover:bg-yellow-900/60 transition-colors duration-300 mx-auto sm:mx-0">
            {icon}
          </div>

          <div className="flex-1 text-center sm:text-left">
            <h3 className="text-lg sm:text-xl md:text-2xl font-semibold text-gray-100 mb-2 sm:mb-3 group-hover:text-yellow-400 transition-colors duration-300">
              {title}
            </h3>
            <p className="text-sm sm:text-base text-gray-400 mb-3 sm:mb-4 leading-relaxed px-2 sm:px-0">{description}</p>
            <a href="#" className="inline-flex items-center text-yellow-400 hover:text-yellow-300 font-medium group-hover:underline text-sm sm:text-base">
...
function AnimatedButton() {
  ...
  return (
    <div
      ref={buttonRef}
      className={`mt-10 md:mt-16 text-center transition-all duration-1000 transform ${
        isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
      }`}
    >
      <button className="px-6 sm:px-8 py-2.5 sm:py-3 bg-gradient-to-r from-yellow-500 to-yellow-600 text-gray-900 font-medium rounded-full hover:from-yellow-400 hover:to-yellow-500 transition-all duration-300 shadow-lg shadow-yellow-500/20 transform hover:-translate-y-1 text-sm sm:text-base">
